...
namespace LWFramework.Core {
    ...
    /**
    public class HotfixManager : IManager, IHotfixManager
    {
        ...
        public IEnumerator IE_LoadScriptBak(HotfixCodeRunMode mode)
        {
            ...
            else
            {
                ...
                var secondPath = FileTool.CombinePaths(Application.streamingAssetsPath, LWUtility.AssetBundles, LWUtility.GetRuntimePlatform(), LWUtility.HotfixFileName);// Application.streamingAssetsPath + "/" + LWUtility.AssetBundles + "/" + LWUtility.GetPlatform() + "/" + LWUtility.HotfixFileName;
                if (!File.Exists(firstPath))
                {
                    var request = UnityWebRequest.Get(secondPath);
                    LWDebug.Log("firstPath:" + firstPath);
        ...
        public async Cysharp.Threading.Tasks.UniTaskVoid LoadScriptAsync(HotfixCodeRunMode mode)
        {
            LWDebug.LogError($"{this} 当前未实现异步加载方式,请使用其他实现");
        ...
        public T Instantiate<T>(string type)
        {
